...
if __name__ == "__main__":
    ...
    for seed in tqdm(SEEDS):
        ...
        for i_episode in tqdm(range(NUM_EPISODES)):
            ...
            for i in range(20):
                with torch.no_grad():
                    ...
                    quality.append(0.0)
                    session_clicked_items.append(selected_doc_feature)
            ...
            for row1 in actual_selected_items:
                for row2 in session_clicked_items:
                    if torch.all(torch.eq(row1, row2)):
                        hit_ratio += 1
            ...
            ep_hit_recall = hit_ratio / len(actual_selected_items)
            ep_hit_precision = hit_ratio / len(session_clicked_items)
            ...
            save_dict["hit_recall"].append(ep_hit_recall)
            save_dict["hit_precision"].append(ep_hit_precision)
        ...
        # wandb.finish()
        directory = f"wp_slate_specialist_lbr"
